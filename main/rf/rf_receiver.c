...
static rf_frame_callback_t s_frame_callback;
...
static void rf_recv_task(void *arg)
{
    ...
    ESP_LOGI(RF_TAG, "RF receiver task running");
...
esp_err_t rf_recv_init(gpio_num_t rx_gpio, RFHandler* rf_rmt) {
    ...
    rf_rmt->rx_gpio = RF_GPIO_UNASSIGNED;
...
esp_err_t rf_recv_start_task(gpio_num_t rx_gpio, RFHandler* rf_rmt)
{
    ...
    if (rf_recv_init(rx_gpio, rf_rmt) != ESP_OK) {
        return ESP_FAIL;
    }

    task_created = xTaskCreate(rf_recv_task,
                               "rf_recv_task",
                               4096,
                               rf_rmt,
                               5,
                               &rf_rmt->rf_recv_handle);
    if (task_created != pdPASS) {
        gpio_isr_handler_remove(rf_rmt->rx_gpio);
        gpio_config_t io_conf_reset = {
            .pin_bit_mask = (1ULL << rf_rmt->rx_gpio),
...
esp_err_t rf_recv_deinit(RFHandler* rf_rmt) {
    ...
    // Return the GPIO to a disabled state now that reception is stopped.
    gpio_config_t io_conf_reset = {
        .pin_bit_mask = (1ULL << rf_rmt->rx_gpio),
        .mode = GPIO_MODE_DISABLE,
        .pull_up_en = GPIO_PULLUP_DISABLE,
    ...
    ESP_ERROR_CHECK(gpio_config(&io_conf_reset));
    rf_rmt->rx_gpio = RF_GPIO_UNASSIGNED;
...
esp_err_t rf_recv_suspend(RFHandler* rf_rmt) {
    ...
    // Remove ISR handler
    ESP_ERROR_CHECK(gpio_set_intr_type(rf_rmt->rx_gpio, GPIO_INTR_DISABLE));
    esp_err_t isr_status = gpio_isr_handler_remove(rf_rmt->rx_gpio);
    if (isr_status == ESP_ERR_INVALID_STATE)
        ESP_LOGI(RF_TAG, "ISR handler already removed for GPIO %d", rf_rmt->rx_gpio);
    else {
...
esp_err_t output_recv(RFHandler* rf_rmt, RFRecvData* recv_data) {
    ...
    ESP_LOGI(RF_TAG, "Protocol: %u", rf_rmt->recv_proto + 1);
    if (recv_data->binary)
    ...
    ESP_LOGI(RF_TAG, "Pulse length: %u us", rf_rmt->recv_delay);
    return ESP_OK;
}
